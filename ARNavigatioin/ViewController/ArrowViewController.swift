...
class ArrowViewController: ARSCNBaseViewController {
    ...
    override func resetOrignalWorld() {
        if needResetOrignalWorld {
            ...
            let t1_matrix4_T = SCNMatrix4MakeTranslation(0.0, 0.0, 0.0)//重新设置中心点的位置 向下 1.5m 向后 1.0m
            ...
            let scene = SCNScene(named: "888.scn")!
            
            scene.rootNode.childNodes.last?.position = SCNVector3Make(0.0, 0.0, -1.0)
//            scene.rootNode.childNodes.last?.rotation = SCNVector4Make(0.0, 0.0, 1.0, -80 / 180 * .pi)
//            scene.rootNode.childNodes.last?.rotation = SCNVector4Make(0.0, 1.0, 0.0, -90 / 180 * .pi)
            
            scene.rootNode.childNodes.last?.eulerAngles.z = -80
            scene.rootNode.childNodes.last?.eulerAngles.x = -90
...
extension ArrowViewController: CLLocationManagerDelegate {
